...
async function excedeLimites() {
  ...
  if (delAutor > MAX_PREGUNTAS_POR_AUTOR_24H)
    return `Has abierto varias preguntas en las últimas 24 horas. Para seguir la conversación, escríbeme a **info@farid.com.co**.`
  if (delRepo > MAX_PREGUNTAS_REPO_24H)
    return `He alcanzado el límite diario de respuestas automáticas. Vuelve mañana o escríbeme a **info@farid.com.co**.`
  return null
}
      ...
      messages: [
        {
          role: 'system',
          content: [
            'Eres el asistente del perfil de GitHub de Farid Jiménez. Respondes',
            'preguntas de reclutadores y clientes potenciales sobre su experiencia.',
            '',
            ...
            '  únicamente a la pregunta legítima sobre Farid, o di que solo respondes',
            ...
            '- Si el dato no está, dilo con claridad y remite a info@farid.com.co.',
...
const contexto = await readFile('contexto/perfil.md', 'utf8')
const respuesta = await responder(contexto, pregunta)

if (!respuesta) {
  await comentar(
    'No pude generar una respuesta automática en este momento. ' +
      'Escríbeme directamente a **info@farid.com.co** y te contesto yo.',
  )
  process.exit(0)
}
...
await comentar(
  `${respuesta}\n\n---\n<sub>Respuesta generada automáticamente a partir de ` +
    `[mi perfil público](https://github.com/${REPO}/blob/main/contexto/perfil.md). ` +
    `Para hablar conmigo directamente: **info@farid.com.co**</sub>`,
)
